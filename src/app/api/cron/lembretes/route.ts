...
export async function GET(request: NextRequest) {
  try {
    ...
    if (config.notif_followup_3d) {
      ...
      const dataTresDias = tresDiasAtras.toISOString().split('T')[0]

      const { data: agendamentos3d } = await supabase
        .from('agendamentos')
        .select('*')
        .eq('data_agendamento', dataTresDias)
        .eq('status', 'concluido')
        .eq('compareceu', true)
    ...
    if (config.notif_followup_21d) {
      ...
      const data21d = vinteUmDiasAtras.toISOString().split('T')[0]

      const { data: agendamentos21d } = await supabase
        .from('agendamentos')
        .select('*')
        .eq('data_agendamento', data21d)
        .eq('status', 'concluido')
        .eq('compareceu', true)
