  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card
          className="bg-gradient-to-br from-blue-800/30 to-red-800/30 border-red-700/50 cursor-pointer hover:scale-105 transition-transform"
          onClick={() => router.push('/dashboard/agendamentos')}
        >
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-sm text-red-300">Agendamentos</div>
                <div className="text-2xl font-bold text-white">{totalAgendamentos}</div>
                <div className="text-xs text-red-400 mt-1">Clique para ver agendamentos</div>
              </div>
        ...
        <Card
          className="bg-gradient-to-br from-red-800/30 to-pink-800/30 border-red-700/50 cursor-pointer hover:scale-105 transition-transform"
          onClick={() => router.push('/dashboard/vendas')}
        >
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-sm text-red-300">Vendas de Produtos</div>
                <div className="text-2xl font-bold text-white">{totalVendas}</div>
                <div className="text-xs text-red-400 mt-1">Clique para ver vendas</div>
              </div>
          ...
          <CardContent>
            <div className="space-y-3">
              {servicosMaisVendidos.map((servico, index) => (
                <div key={servico.nome} className="flex items-center justify-between p-3 bg-red-700/30 rounded-lg">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-red-300">{servico.quantidade} vezes</div>
                    </div>
                  </div>
            ...
            <div className="space-y-3">
              {produtosMaisVendidos.length > 0 ? (
                produtosMaisVendidos.map((produto, index) => (
                  <div key={produto.nome} className="flex items-center justify-between p-3 bg-red-700/30 rounded-lg">
                    <div className="flex items-center space-x-3">
                      <div className="w-8 h-8 bg-red-600 rounded-full flex items-center justify-center text-white font-bold">
                        {index + 1}
                      </div>
                      <div>
                        ...
                        <div className="text-sm text-red-300">{produto.quantidade} unidades</div>
                      </div>
                    </div>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-red-700/30 p-4 rounded-lg border border-red-600/50">
              <div className="text-3xl font-bold text-red-400">{agendamentosPorStatus.agendado}</div>
              <div className="text-sm text-red-300">Agendado</div>
            </div>
            ...
            <div className="bg-red-700/30 p-4 rounded-lg border border-red-600/50">
              <div className="text-3xl font-bold text-red-400">{agendamentosPorStatus.concluido}</div>
              <div className="text-sm text-red-300">Concluído</div>
            </div>
            <div className="bg-red-700/30 p-4 rounded-lg border border-red-600/50">
      ...
      <Card className="bg-red-800/30 border-red-700/50">
        ...
        <CardContent>
          {agendamentosComparecidos.length > 0 ? (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-red-700/30 border-b border-red-600/50">
                  <tr>
                    <th className="text-left p-3 text-red-300">Data</th>
                    <th className="text-left p-3 text-red-300">Cliente</th>
                    <th className="text-left p-3 text-red-300">Barbeiro</th>
                    <th className="text-left p-3 text-red-300">Serviços</th>
                    <th className="text-right p-3 text-red-300">Valor</th>
                  </tr>
                </thead>
                <tbody>
                  {agendamentosComparecidos.map((agendamento) => {
                    ...
                    return (
                      <tr key={agendamento.id} className="border-b border-red-700/30 hover:bg-red-700/20">
                        <td className="p-3 text-white">{agendamento.data_agendamento}</td>
                        <td className="p-3 text-white">{agendamento.nome_cliente}</td>
                        <td className="p-3 text-red-300">{barbeiro?.nome || agendamento.Barbeiro || '-'}</td>
                        <td className="p-3 text-red-200">
                          {servicosAgendamento.length > 0 ? (
                            <div className="flex flex-wrap gap-1">
                              {servicosAgendamento.map((servico, idx) => (
                                <span key={idx} className="bg-red-600/30 px-2 py-0.5 rounded text-xs">
                                  {servico}
                                </span>
                              ))}
                            </div>
                          ) : (
                            <span className="text-zinc-400">Sem serviços</span>
                          )}
                        </td>
                        <td className="p-3 text-right text-green-400 font-bold">
                ...
                <tfoot className="bg-red-700/30 border-t border-red-600/50">
                  <tr>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card className="bg-red-800/30 border-red-700/50">
          <CardContent className="p-4">
            <div className="flex items-center space-x-2 mb-2">
              <Package className="w-5 h-5 text-red-400" />
              <div className="text-sm text-red-300">Faturamento Produtos</div>
            </div>
